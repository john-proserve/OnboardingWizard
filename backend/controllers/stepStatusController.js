...
const getStepStatusById = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json({ message: `ID: ${stepStatus._id} Step Status: ${step.status}` });
});

// Get all steps by user
const getStepStatusByUser = asyncHandler(async (req, res) => {
  const { _id } = req.params;
  if (!_id) {
    return res.status(400).json({ message: 'User ID Required' });
  }

  const userSteps = await StepStatus.find({ user_id: { _id } }).populate([
    'step_id',
    'status_id',
  ]);
  if (!userSteps) {
    return res.status(404).json({ message: 'No status for user' });
  }

  const result = userSteps.map((step) => {
    return {
      id: step.id,
      step: step.step_id.step_title,
      status: step.status_id.status_title,
    };
  });
...
const createNewStepStatus = asyncHandler(async (req, res) => {
  ...
  if (!newStepStatus) {
    return res
      .status(400)
      .json({ message: 'Invalid Step Status data received.' });
  }
  res
    .status(201)
    .json({ message: `New Step Status ${newStepStatus._id} created.` });
});

// Update Note
const updateStepStatus = asyncHandler(async (req, res) => {
  const { _id, user_id, step_id, status_id, date_started, date_completed } =
    req.body;
